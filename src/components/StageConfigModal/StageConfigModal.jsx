    ...
    >
      ...
      <form id="stage-config-form" className={styles.form} onSubmit={handleSave}>
        ...
        <div className={styles.formGroup}>
          ...
          {/* Dropdown only -- no free-text fallback (rbr-rally-creator-web#44:
              "Have the weather as a dropdown only no free form text!!").
              options.weatherOptions is still only a few examples observed
              for one stage/leg combo, not a confirmed exhaustive list (see
              the backend's rallyOptions.js / rbr-rally-creator-service#16),
              so an existing stage or restored draft can carry a
              tracksettings_id outside the current list. Rather than let a
              plain select silently mis-render that value as unselected, it
              gets appended as an extra, clearly-marked option so it stays
              visibly selected and isn't overwritten just by the user
          ...
          >
            ...
            {draft.tracksettings_id && !options.weatherOptions?.includes(draft.tracksettings_id) && (
              <option value={draft.tracksettings_id}>{draft.tracksettings_id} (not in current list)</option>
            )}
          </select>
        </div>
